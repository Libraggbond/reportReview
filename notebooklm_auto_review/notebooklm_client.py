...
class NotebookLMClient:
    ...
    async def get_sources(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            sources = []
            # 尝试多种可能的选择器
            source_elements = await self.page.query_selector_all(
                '[data-source], [role="listitem"]:has-text(".docx"), [role="listitem"]:has-text(".pdf"), '
                '.source-item, [aria-label*=".docx"], [aria-label*=".pdf"]'
            )
            ...
            for elem in source_elements:
                try:
                    ...
                    if name:
                        ...
                        selected = False
                        try:
                            selected = await elem.evaluate(
                                "el => el.getAttribute('aria-selected') === 'true' || el.classList.contains('selected')"
                            )
                        except:
                            pass
